document.addEventListener("DOMContentLoaded", function () {
    ...
    async function openModal(project) {
        ...
        const hasExternalGallery = hasGallery && gallerySourceType !== "fallback";
        if (project.driveLink && !hasExternalGallery) {
            modalButton.href = project.driveLink;
            modalButton.classList.remove("hidden");
            modalButton.innerHTML = `<i class='bx bx-show'></i> Voir le r&eacute;sultat`;
    ...
    async function fetchGalleryManifest(folder) {
        ...
        try {
            ...
            console.info('Galerie introuvable pour', base);
        }
        return [];
    }
